...
public class DatabaseOperation {
    ...
    public void insertCustomer(UserInfo user) throws SQLException {
        try {
            String insertQuery = "insert into userInfo" + "('" + "name" + "'," + "'" + "address" + "','" + "phone"
                    + "','" + "type" + "')" + " values('" + user.getName() + "','" + user.getAddress() + "'" + ",'"
                    + user.getPhone_no() + "'" + ",'" + user.getType() + "'" + ")";
    ...
    public void updateCustomer(UserInfo user) {
        ...
        try {
            String updateQuery = "update userInfo set name = '" + user.getName() + "'," + "address = '"
                    + user.getAddress() + "'," + "phone = '" + user.getPhone_no() + "'," + "type = '" + user.getType()
                    + "' where user_id= " + user.getCustomer_id();
    ...
    public ResultSet getAvailableRooms(long check_inTime) {
        try {
            ...
            /// SELECT room_no FROM room LEFT OUTER JOIN booking ON room.room_no =
    ...
    public ResultSet getBookingInfo(long start_date, long end_date, String roomNo) {
        try {
            /*
             * 
             * select * from booking where (check_in <= start_date and (check_out=0 or
             * check_out<= end_date ) ) or (check_in>start_date and check_out< end_date) or
             * (check_in <=end_date and (check_out =0 or check_out > end_date) )
             * 
             */

            String query = "select * from booking where booking_room = '" + roomNo + "' AND (" + "( check_in <= "
                    + start_date + " and ( check_out = 0 or check_out<= " + end_date + ") ) or" + "( check_in >"
                    + start_date + " and check_out< " + end_date + " ) or" + "( check_in <= " + end_date
                    + " and ( check_out =0 or check_out> " + end_date + ") ) )";
